...
namespace Febris.UserNode.DataAccessLayer.Queries
{
    ...
    public class WidgetQueries : IWidgetQueries
    {
        ...
        public WidgetQueries(IHttpContextAccessor httpContextAccessor)
        {
            _federation = Febris.ModelLibrary.ViewModels.HubFederationSettings.Resolve(StaticDetails.PassedBackConfig, Febris.SharedServices.JwtSigningKeyProvider.IsUnsubstitutedTemplate);
            _endpoint = _federation.DataApi;
            _tokenContext = new TokenQueries();
        ...
        public WidgetQueries(IHttpContextAccessor httpContextAccessor, Febris.ModelLibrary.ViewModels.IHubFederationSettings federation)
        {
        ...
        #region Requests
        private async Task<byte[]> MakeByteGetRequest(string method, string dataPackage)
        {
            try
            {
                ...
                {
                    endPoint = _endpoint + "Widget/" + method,
        ...
        private async Task<string> MakeGetRequest(string method, string dataPackage)
        {
            ...
            {
                Febris.SharedServices.FebrisLog.Error(ex, "WidgetQueries.MakeGetRequest: suppressed exception");
                return ex.Message;
            }
        }
        private async Task<string> MakePostRequest(string method, string dataPackage)
        {
            ...
            {
                Febris.SharedServices.FebrisLog.Error(ex, "WidgetQueries.MakePostRequest: suppressed exception");
                return ex.Message;
            }
        }
        private async Task<string> MakePutRequest(string method, string dataPackage)
        {
            ...
            {
                Febris.SharedServices.FebrisLog.Error(ex, "WidgetQueries.MakePutRequest: suppressed exception");
                return ex.Message;
            }
        }
        #endregion
        public async Task<byte[]> RemoteImageLoader(string input)
        {
            byte[] output = { };
            ...
            try
            {
                ...
                method = "GetImage?input=" + input;
                //method = "GetMarketplaceListingImage/" + input;
                output = await MakeByteGetRequest(method, dataPackage);
                //output = result;
            }
            catch (Exception ex)
            {
